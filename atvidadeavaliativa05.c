...
void ordenarPorDistancia(Entidade *jogador)
{
    ...
    {
        int maisProximoIdx = i;
        float menorDistancia = 0.0f;

        for (int j = i; j < totalEntidades; j++)
        {
            float dx = vetorEntidades[j]->pos.x - jogador->pos.x;
            float dy = vetorEntidades[j]->pos.y - jogador->pos.y;
            float distancia = sqrtf(dx * dx + dy * dy);

            if (j == i || distancia < menorDistancia)
            {
        ...
        if (maisProximoIdx != i)
        {
            ...
            vetorEntidades[i] = vetorEntidades[maisProximoIdx];
            vetorEntidades[maisProximoIdx] = tmp;
        }
    }
}
