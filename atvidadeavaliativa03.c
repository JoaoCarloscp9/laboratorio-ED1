...
void aplicarItem(Jogador *j, Item *item) {
    switch (item->tipo) {
        ...
        case ITEM_POCAO:
            ...
            break;
        case ITEM_ESCUDO:
            j->escudo += item->dados.escudo.absorcao;
            break;
    }
    item->coletado = true;
}
...
void desenharItem(Item *item) {
    ...
    Color cor;
    switch (item->tipo) {
        case ITEM_ARMA:  cor = RED;    break;
        case ITEM_POCAO: cor = (item->dados.pocao.cura < 0) ? PURPLE : GREEN; break;
        case ITEM_ESCUDO: cor = BLACK;  break;
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        BeginDrawing();
            ...
            DrawText(TextFormat("Vida: %d   Dano: %.1f   Armadura: %d", jogador.vida, jogador.dano, jogador.escudo), 10, 10, 22, DARKGRAY);
            DrawText("Vermelho = arma (aumenta dano) | Verde = pocao (cura) | BLACK = escudo (armadura)", 10, 34, 18, GRAY);
